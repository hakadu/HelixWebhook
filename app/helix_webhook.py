...
def push_event(body):
    ...
    url = 'https://' + helix['helix.host'] + ":" + helix['helix.port'] + '/tsws/api/v1.0/events'
...
def build_event_body(data):
    ...
    for alert in data['alerts']:
        ...
        if alert['annotations']:
            if alert['annotations'].get('message'):
        ...
        if data['status'].lower() == 'resolved':
            status = 'CLOSE'
        if alert['labels'].get('alert_name'):                                                                                                                                                                                 
          alert_name = alert['labels'].get('alert_name')                                                                                                                                                                      
        else:                                                                                                                                                                                                                 
        ...
        body = {'source_identifier': data['receiver'], 'object_id': data['receiver'], 'object_class': 'Prometheus',
                ...
                'object': alert_name, 'msg': msg, 'status': status, 'url': alert['generatorURL'],
